...
import com.github.fujianlian.klinechart.config.KLineConstant;
import com.github.fujianlian.klinechart.R;
import com.github.fujianlian.klinechart.BaseKLineChartView;
import com.github.fujianlian.klinechart.KLineChartView;
import com.github.fujianlian.klinechart.base.BChartDraw;
 ...
 */
public class MainDraw extends BChartDraw<ICandle> {
    ...
    public MainDraw(BaseKLineChartView view) {
        ...
        mTextPaint.setColor(ContextCompat.getColor(context, R.color.chart_white));
        mSelectorFramePaint.setColor(ContextCompat.getColor(context, R.color.chart_white));
...
//            Log.i(KLineConstant.TAG,"2.drawK  drawTranslated:"+view.getmStopIndex());
//        }
    }
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        try {
            ...
            listLeft.add(KLineConstant.T_DATE);
            listLeft.add(KLineConstant.T_HIGH);
            listLeft.add(KLineConstant.T_LOW);
            listLeft.add(KLineConstant.T_OPEN);
            listLeft.add(KLineConstant.T_CLOSE);

            ArrayList<String> listRight = new ArrayList<>();
//        view.getAdapter().getItem(index)
//            listRight.add(DateFormatter.formatDate(view.getAdapter().getTime(index)));
            listRight.add(view.getAdapter().getDate(index));
            listRight.add(point.getHighPrice() + "");
            listRight.add(point.getLowPrice() + "");
            listRight.add(point.getOpenPrice() + "");
            listRight.add(point.getClosePrice() + "");
            //量能不为0添加
//            if (point.getVol() > 0) {
//                listLeft.add(KLineConstant.T_VOL);
//                listRight.add(point.getVol() + "");
//            }
            //求出最大的宽度
            float w1 = 0f, w2 = 0f;
            for (String sl : listLeft) {
                w1 = Math.max(w1, mTextPaint.measureText(sl));
            }
            for (String sr : listRight) {
                w2 = Math.max(w2, mTextPaint.measureText(sr));
            }
//        if (KLineConstant.IS_LOG){
//            Log.i("kline0003","w1:"+w1+"\tw2:"+w2+"\tmargin:"+margin
//            +"\t"+mTextPaint.measureText(view.getAdapter().getDate(index)));
//        }
            //每个间距
            float each = 10f;
            //宽度
            float width = w1 + margin * 2 + w2;
            //高度
            float height = textHeight * listLeft.size() + each * listLeft.size() - 2;
            //左侧 还是右侧
            ...
            canvas.drawRect(new RectF(left, top, left + width, top + height + each), mSelectorBackgroundPaint);
            canvas.drawRect(new RectF(left, top, left + width, top + height + each), mSelectorFramePaint);

            float y1 = top + textHeight;
            //画文本
            for (int i = 0; i < listLeft.size(); i++) {
                canvas.drawText(listLeft.get(i), left + margin / 3, y1, mTextPaint);
                canvas.drawText(listRight.get(i), left + width - mTextPaint.measureText(listRight.get(i)) - margin / 3, y1, mTextPaint);
                y1 = y1 + textHeight + each;
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void setLine(boolean line) {
        if (isLine != line) {
            ...
            if (isLine) {
                //分时
                kChartView.setCandleWidth(kChartView.dp2px(2f));
            } else {
                //k 线
