...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private Paint mLastRowPaint  = new Paint(Paint.ANTI_ALIAS_FLAG);
//    private Paint mRoundPaint  = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mRoundSolidPaint  = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mTextDynamicPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private void init() {
        ...
        mLastRowPaint.setColor(Color.YELLOW);
        mLastRowPaint.setPathEffect(new DashPathEffect(new float[]{4, 4}, 0));
//        mRoundPaint.setColor(Color.YELLOW);
//        mRoundPaint.setColor(Color.YELLOW);
        mLastRowPaint.setStyle(Paint.Style.STROKE);
        mTextDynamicPaint.setColor(Color.YELLOW);
        mTextDynamicPaint.setTextSize(dp2px(10));
        mRoundSolidPaint.setStrokeWidth(2);
//        mRoundSolidPaint.setColorFilter()
        mRoundSolidPaint.setColor(Color.parseColor("#191c28"));
        mRoundSolidPaint.setStyle(Paint.Style.FILL);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        this.mWidth = w;
//        if (KLineConstant.IS_LOG){
//            Log.i(KLineConstant.TAG,"3.onSizeChanged  w:"+w+"\tmScrollX:"+mScrollX);
//        }
        singleSpace = mWidth / mGridColumns;
...
//        if (KLineConstant.IS_LOG){
//            Log.i(KLineConstant.TAG,"区域大小 initRect .onSizeChanged  isShowChild:"+isShowChild+"\tmWidth:"+mWidth);
//        }
        if (isShowChild) {
     ...
     * @param canvas
     */
    private float singleSpace = 0; //单个间距
...
//            float currentPointX = getX(i)-singleSpace ;
            Object lastPoint = i == 0 ? currentPoint : getItem(i - 1);
            float lastX = i == 0 ? currentPointX : getX(i - 1);
//            float lastX = i == 0 ? currentPointX-singleSpace : getX(i - 1)-singleSpace;
            if (mMainDraw != null) {
...
//            if (i == mStopIndex){
//                canvas.drawRect(lastX,100,lastX+singleSpace,120,mLastRowPaint);
//            }
        }
    ...
    private void drawText(Canvas canvas) {
        ...
        if (mMainDraw != null) {
            ...
            if (!TextUtils.isEmpty(price)){
                //起始位置 = 宽度 - 文字宽度
                Paint.FontMetrics fm2 = mTextDynamicPaint.getFontMetrics();
                float h = fm2.descent - fm2.ascent;
//                float h = fm2.bottom - fm2.top;
                float w = mTextDynamicPaint.measureText(price);
                double v = Double.parseDouble(price);
                double min = mMainMinValue;
                double max = mMainMaxValue;
//                float dynamicEndX = mMainRect.right - w ; //线最后的位置
                float dynamicY = 0f;
                if (v  > max){
//                    //大于最大价
                    dynamicY = mMainRect.top + h;
                }else if (v < min){
//                    //小于最小价
                    dynamicY = mMainRect.bottom - h;
                }else {
//                  //最大位置除以最小位置
                    //距离= 最大位置 -最小位置  / 最大金额-最小金额
                    float distant = (float) (Math.abs(mMainRect.top - mMainRect.bottom -h*2) / Math.abs(max - min));
                    //动态位置= 最大金额 - 当前金额 * 实时距离
                    dynamicY = (float) (Math.abs(max - Double.parseDouble(price) ) * distant);
//                    if (KLineConstant.IS_LOG){
//                        Log.i("baseK0000003","bottom:"+mMainRect.bottom+"\ttop:"+mMainRect.top
//                                +"\tw :"+w+"\th:"+h+"\t"+mMainRect.right+"\tleft:"+mMainRect.left
//                                +"\tprice:"+price  +"\tdistant:"+distant+"\tdynamicY:"+dynamicY
//                        +"\nmax:"+max+"\tmin:"+min);
//                    }
                }
                float margin = 10;
                ...
                rectF.left = mMainRect.right - singleSpace;
                rectF.right = mMainRect.right - margin;
                rectF.top = dynamicY - h ;
                rectF.bottom = dynamicY + h ;
                //实心
                RectF rectSolid = new RectF();
                rectSolid.left = mMainRect.right - singleSpace;
                rectSolid.right = mMainRect.right - margin ;
                rectSolid.top = dynamicY - h ;
                rectSolid.bottom = dynamicY + h  ;
                canvas.drawLine(0,dynamicY,rectF.left - 20,dynamicY,mLastRowPaint);
                canvas.drawRoundRect(rectSolid,30,30,mRoundSolidPaint);
                canvas.drawRoundRect(rectF,30,30,mLastRowPaint);
                canvas.drawText(price,rectF.left + (rectF.right - rectF.left - w) /2  , rectF.top + h +h/4 ,mTextDynamicPaint);
            }
        }
...
//            mDataLen = (mItemCount - 1) * mPointWidth;
            mDataLen = (mItemCount - 1) * mPointWidth+singleSpace;
//            if (KLineConstant.IS_LOG){
//                Log.i(KLineConstant.TAG,"1.mDataLen:"+mDataLen +"\tmItemCount:"+mItemCount
//                +"\tmScrollX:"+mScrollX+"\tmPointWidth:"+mPointWidth+"\tsingleSpace:"+singleSpace);
//            }
            checkAndFixScrollX();
            setTranslateXFromScrollX(mScrollX);
        } else {
...
//        if (KLineConstant.IS_LOG){
//            Log.i(KLineConstant.TAG,"onScrollChanged:"+mScrollX);
//        }
        setTranslateXFromScrollX(mScrollX);
    }
     ...
     * 绘制 最后一个空格的区域
     */
    public void drawLastRow(Canvas canvas ,float startX){
//        canvas.draw
    }
